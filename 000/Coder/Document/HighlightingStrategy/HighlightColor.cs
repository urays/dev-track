...
namespace IDevTrack.Coder.Document
{
	...
	public class HighlightColor
	{
		...
		static Color ParseColorString(string colorName)
		{
		...
		{
			Debug.Assert(el != null, "IDevTrack.Coder.Document.SyntaxColor(XmlElement el) : el == null");
			...
			if (el.Attributes["color"] != null) {
				...
				if (c[0] == '#') {
					...
					color = (Color)(Color.GetType()).InvokeMember(c, BindingFlags.GetProperty, null, Color, Array.Empty<object>());
				}
				hasForeground = true;
			} else {
			...
			if (el.Attributes["bgcolor"] != null) {
				...
				if (c[0] == '#') {
					...
					backgroundcolor = (Color)Color.GetType().InvokeMember(c, BindingFlags.GetProperty, null, Color, Array.Empty<object>());
				}
				hasBackground = true;
			}
		}
