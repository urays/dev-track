...
namespace CCS.Catcher.Internal
{
    ...
    public class DataPack
    {
        ...
        private double _max = -Catcher.Utils.Basic.DOUBLE_DELTA; //DataList 初始化时默认包含 {0,0}
        private double _min = Catcher.Utils.Basic.DOUBLE_DELTA;
        private int _remain_digit = 2;  //数据小数点后保留位数

        public double ldratio { get; set; } = 1.0f; //layout / data_value

        public double sDataMax => Math.Round(_max, _remain_digit);

        public double sDataMin => Math.Round(_min, _remain_digit);

        public double sDataMid => Math.Round((sDataMax + sDataMin) / 2, _remain_digit);

        public double sDataRange => Math.Round(sDataMax - sDataMin, _remain_digit);

        public int sCount => sPacks.Count;

        public int sMaxCount { get; private set; } = 3;

        public bool canSelect => sCount < sMaxCount;

        public int remDigit
        {
            get => _remain_digit;
            set => _remain_digit = Math.Min(value, Catcher.Utils.Basic.DOUBLE_MAX_DIGIT);
        }
        ...
        {
            foreach (string name in sPacks.Keys)
            {
                Packs[name].ColorNum = DefaultColorNum;
                Packs[name].Datas.Clear();
            }
        ...
        private void GlobalMaxMinReset()
        {
            ...
            {
                _max = -Catcher.Utils.Basic.DOUBLE_DELTA;
                _min = Catcher.Utils.Basic.DOUBLE_DELTA;  //以_max < _min 作为强制更新条件
            }
        }

        public string GetName(int index)
        {
            if (index < 0 || index >= sPacks.Count)
            {
                return "";
            }
        ...
        private Color DataColor(int index)
        {
            if (index < 0 || index >= sPacks.Count)
            {
                return COLORPACK[DefaultColorNum];
            }
            else
            {
                return COLORPACK[sPacks.ElementAt(index).Value];
            }
        }
