...
{
    public partial class ISTScope : UserControl
    {
        //添加设置选项
        ...
        private readonly Catcher.Internal.DataPack DPK = new Catcher.Internal.DataPack(5); //数据包
        ...
        private void ISTScope_Load(object sender, EventArgs e)
        {
            ...
            //初始化参数设置表
            foreach (Tuple<string, bool> item in SetItems)
            {
        ...
        {
            if (DPK.sDataRange < Catcher.Utils.Basic.DOUBLE_DELTA)
            {
        ...
        public void AddItems(string pname)
        {
            ...
            {
                int tmpw = (int)(g.MeasureString(pname, pramBox.Font).Width) + 1;
                int tmph = (int)(g.MeasureString(pname, pramBox.Font).Height);
        ...
        {
            string fmt = "f" + DPK.remDigit.ToString();
        ...
        {
            int index = pramBox.IndexFromPoint(e.Location);
            if (index != ListBox.NoMatches)
            {
                string item = pramBox.SelectedItem.ToString();
        ...
        private void Paper_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            {
                float tmp = mv_wave_layout_midval + (e.Y - mv_start.Y);
