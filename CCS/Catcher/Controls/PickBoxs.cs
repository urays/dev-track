...
namespace CCS.Catcher.Controls
{
    public class PickBoxs : IDisposable
    {
        ...
        private readonly int min_w = 100;
        private readonly int min_h = 100;
        private readonly Control m_control = null;
        ...
        public PickBoxs(UserControl ctl, int minw, int minh)
        {
            min_w = minw;
            min_h = minh;
            for (int i = 0; i < 8; i++)
            ...
            if (m_control != null)
            {
                ...
                // m_control.ToFrontLater += new IControl.delegateBringToFrontLater(ControlToFrontLater);
