...
double _i2sqrt(int number)
{
	...
	while (l <= r)
	{
		...
		if (f < c && c - f > EPSINON) l = mid;
		else if (f > c && f - c > EPSINON) r = mid;
		else {
			return mid;
		}
	}
...
double _icos(double rad)
{
	...
	return _isin(rad)*flag;
}

static double fast_atan_table[257] =
{
...
static double fast_atan2(float y, float x)
{
...
double _iatan_r(float x, float y)
{
	return fast_atan2(y, x);
}

double _iatan_d(float x, float y)
{
	return fast_atan2(y, x) / PII * 180.;
}

//  辗转相除法 求最大公因数(c)
//  证明：设两数 x,y, 令 x > y; c = (x,y)
//  x = ay + r; 若 r = 0, 则x,y的最大公因数为y;
//  容易看出 0 <= r < y;
//  若r <> 0, x = mc, y = nc (n 和 m 互质)  mc = anc + r -> r = (m - an)*c -> c为r的因数
//  通过反证法可以证得 (m - an) 与 n,m 两两互质
//  假设(m - an) 与 n有公因数 t, ->（m-an）= wt,n = kt (w,k属于R) -> m = wt + akt, n = kt
//  -> x = (w+ak)tc , y=ktc -> (x,y) = tc, 若t > 1 则tc > c, 与实际上c是最大公因数矛盾.
//  所以 （x,y）= (y,r)
